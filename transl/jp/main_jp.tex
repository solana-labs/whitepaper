...
このようなスケーリング方法は可用性を犠牲にして成り立ってる。$10 \times 1$~gbpsのネットワーク環境の稼働率が0.999のとき、ネットワーク全体の稼働率は\(0.999^{10} = 0.99 \)となる。
