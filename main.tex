\title{Solana: A new architecture for a high performance blockchain v0.8.10 }
...
As shown in Figure~\ref{fig:design}, Leader is designed to generate a Proof of History. It sequences user messages. It orders them such that they can be efficiently processed by maximizing throughput.  It executes the transactions on the current state that is stored in RAM. It publishes the transactions and a signature of the state to the replications nodes called Verifiers. Verifiers execute the same transactions on their copies of the state, and publish their computed signatures of the state as confirmations. The published confirmations serve as votes for the consensus algorithm.

There are many Leaders in the network, and each Verifier node has the same hardware capabilities as a Leader and can be elected as a Leader.  A Primary Leader and lower ranked Leaders are elected via a consensus algorithm. Elections for the proposed PoS algorithm are covered in depth in Section~\ref{subsec:elections}.

The Leader generates a Proof of History sequence, which provides global read consistency and a verifiable passage of time for the network.
...
\begin{figure}
  \begin{center}
    ...
    \caption[Fig 10]{Leader network limits\label{fig_10}}
  \end{center}
  \end{figure}
